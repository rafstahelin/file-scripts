...
if [ -f "${TOKEN_NAME_VERSION}/multidatabackend.json" ]
then
    if command -v jq >/dev/null 2>&1
    then
        # Use jq if available
        tmp_file=$(mktemp)
        jq --arg ds "datasets/$DATASET_DIR" \
        ...
        walk(
            if type == "object" then
                if .id == "text_embeds" then
                    # For text_embeds section, only update cache_dir
                    del(.instance_data_dir) |
                    del(.cache_dir_vae) |
                    .cache_dir = $ct
                elif .id == "1024" or .id == "768" or .id == "512" then
                    # Remove cache_dir if it exists
                    del(.cache_dir) |
                    # Update paths with combined token+dataset naming
        ...
        # First remove any cache_dir lines that appear after disabled
        sed -i '/"disabled":/,/^[[:space:]]*}/{/[[:space:]]*"cache_dir":/d}' "${TOKEN_NAME_VERSION}/multidatabackend.json"
        
        # Then update the paths
        sed -i \
            -e 's|\("instance_data_dir":[[:space:]]*"\)[^"]*\(".*\)|\1datasets/'"$DATASET_DIR"'\2|g' \
            -e 's|\("cache_dir_vae":[[:space:]]*"\)[^"]*\(".*\)|\1'"$VAE_CACHE_DIR"'/1024\2|g' \
            "${TOKEN_NAME_VERSION}/multidatabackend.json"

        # Update text_embeds section
        sed -i '/"id"[[:space:]]*:[[:space:]]*"text_embeds"/,/^[[:space:]]*}/{
            s|\("cache_dir":[[:space:]]*"\)[^"]*\(".*\)|\1'"$TEXT_CACHE_DIR"'\2|g
...
# Update config.json
if [ -f "${TOKEN_NAME_VERSION}/config.json" ]
then
    sed -i \
...
if [ -f "${TOKEN_NAME_VERSION}/user_prompt_library.json" ]
then
    sed -i \
