...
class LoRaMover:
    ...
    def sync_to_dropbox(self, model_path: str, is_single_version: bool = False) -> None:
        """Sync processed files to Dropbox using rclone with detailed progress."""
        try:
            ...
            # Now run the actual transfer with progress
            cmd = [
                "rclone",
                "copy",
                "--checksum",
            ...
            rprint(f"[blue]Running command: {' '.join(cmd)}[/blue]")
            ...
            with Progress(
                ...
                # Create current file progress task
                ...
                while True:
                    ...
                    if output:
                        ...
                        if line.endswith(".safetensors"):
                            current_file = line
                            rprint(f"\n[yellow]Processing: {current_file}[/yellow]")
                            progress.update(file_task, completed=0, visible=True,
                                         description=f"[yellow]{current_file}")
                        ...
                        # Check for progress percentage
                            ...
                            try:
                                ...
                                if percent == 100:
                                    ...
                                    rprint(f"[green]Completed: {current_file}[/green]")
                                    current_file = ""
                        ...
                        elif any(x in line.lower() for x in ['error', 'warning', 'skip']):
