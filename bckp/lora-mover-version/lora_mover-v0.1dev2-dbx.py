...
class LoRaMover:
    ...
    def sync_to_dropbox(self, model_path: str, is_single_version: bool = False) -> None:
        ...
        try:
            ...
            destination = "dbx:/studio/ai/libs/SD/loras/flux"
            ...
            rprint(f"[yellow]Syncing files to Dropbox...[/yellow]")
            ...
            # Show progress while rclone is running
            with Progress(
                TextColumn("[bold blue]{task.description}"),
                BarColumn(complete_style="green"),
                TaskProgressColumn(),
                ...
                task = progress.add_task("[cyan]Uploading to Dropbox...", total=None)
                ...
                while True:
                    ...
                    if output:
                        ...
                        progress.update(
                            task, 
                            description=f"[cyan]Uploading: {output.strip()}"
                        )
                
                return_code = process.poll()
                
                if return_code == 0:
                    rprint("[green]Dropbox synchronization completed successfully![/green]")
                else:
                    rprint("[red]Error during Dropbox synchronization[/red]")
